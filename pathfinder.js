...
function clean(bot, n, x=0, y=0, z=0) {
    ...
    return b && (b.displayName.includes('Air') || b.material === 'plant' || ghostBlocks.includes(b.name));
}
...
function gridWalk(bot, goal) {
    ...
    if (goal && botPos.distanceTo(botPos.offset(goal.x, goal.y, goal.z)) >= 0.1) {
        ...
        bot.setControlState('jump', goal.y > 0.5);
    } else {
...
function pathfind(bot, start, end, range=2, maxLoops=100) {
    let openList = [];
    let closedList = [];
    ...
    let searchSpace = [...openList, ...closedList];

    if (searchSpace.length) {
        let point = searchSpace.reduce((p, c)=>{
            return p.f < c.f ? p : c;
        });
        let path = [startNode];
        while (point.root) {
